...
class GenericDataset(data.Dataset):
    def __init__(self, dataset_name, split, random_sized_crop=False):
        ...
        if self.dataset_name == 'imagenet':
            ...
            # if self.split != 'train':
            #     transforms_list_augmentation = [transforms.Resize(72),
            ...
            #         transforms_list_augmentation = [transforms.Pad(72),
            ...
            self.transform_augmentation_normalize = transforms.Compose(
                transforms_list_augmentation + transforms_list_normalize)
            ...
            if self.split == 'val':
                split_data_dir = env.IMAGENET_DIR + '/train'
                self.classes = datasets.ImageFolder(split_data_dir, self.transform_augmentation_normalize).classes
                val_annot_dir = env.IMAGENET_DIR + '/' + self.split + '/val_annotations.txt'
                test_file = open(val_annot_dir, "r")
                self.val_annot = test_file.readlines()

                for line_ in self.val_annot:
                    annot = line_.split('\t')
                    self.label_val[annot[0]] = annot[1]
            ...
            self.transform_augmentation_normalize = transforms.Compose(
                transforms_list_augmentation + transforms_list_normalize)
            self.data = Places205(root=env.PLACES205_DIR, split=self.split,
                                  transform=self.transform_augmentation_normalize)
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            img, label = self.data[index]
            img_name = self.data.imgs[index][0].split('\\')[2]
            label = self.label_val[img_name]
            c_label = self.classes.index(label)
            return img, int(c_label), index
