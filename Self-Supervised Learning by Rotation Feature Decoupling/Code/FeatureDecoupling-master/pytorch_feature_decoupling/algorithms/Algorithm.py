...
class Algorithm():
    ...
    def _get_net_checkpoint_filename(self, net_key, epoch):
        return os.path.join(self.exp_dir, net_key+'_net_epoch_exp2'+str(epoch))

    def _get_optim_checkpoint_filename(self, net_key, epoch):
        return os.path.join(self.exp_dir, net_key+'_optim_epoch_exp2'+str(epoch))

    def _get_cri_checkpoint_filename(self, net_key, epoch):
        return os.path.join(self.exp_dir, net_key+'_cri_epoch_exp2'+str(epoch))
    ...
    def solve(self, data_loader_train, data_loader_test):
        ...
        for self.curr_epoch in range(start_epoch, self.max_num_epochs):
            ...
            # conv1_tr.append(train_stats['prec1_conv1'])
            # conv2_tr.append(train_stats['prec1_conv2'])
            # conv3_tr.append(train_stats['prec1_conv3'])
            # conv4_tr.append(train_stats['prec1_conv4'])
            # conv5_tr.append(train_stats['prec1_conv5'])
            # loss_tr.append(train_stats['loss'])
            ...
            if (data_loader_test is not None):
                ...
                # conv1_ts.append(eval_stats['prec1_conv1'])
                # conv2_ts.append(eval_stats['prec1_conv2'])
                # conv3_ts.append(eval_stats['prec1_conv3'])
                # conv4_ts.append(eval_stats['prec1_conv4'])
                # conv5_ts.append(eval_stats['prec1_conv5'])
                # loss_ts.append(eval_stats['loss'])

        self.print_eval_stats_of_best_model()
        # np.save('conv1_tr', conv1_tr)
        # np.save('conv2_tr', conv2_tr)
        # np.save('conv3_tr', conv3_tr)
        # np.save('conv4_tr', conv4_tr)
        # np.save('conv5_tr', conv5_tr)
        # np.save('loss_tr', loss_tr)
        # np.save('conv1_ts', conv1_ts)
        # np.save('conv2_ts', conv2_ts)
        # np.save('conv3_ts', conv3_ts)
        # np.save('conv4_ts', conv4_ts)
        # np.save('conv5_ts', conv5_ts)
        # np.save('loss_ts', loss_ts)
    ...
    def run_train_epoch(self, data_loader, epoch):
        ...
        for idx, batch in enumerate(tqdm(data_loader(epoch), total=len(data_loader))):
            iteration_no = epoch*len(data_loader) + idx
            train_stats_this = self.train_step(batch, iteration_no)
