...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        self.maxPool3d_5a_2x2 = MaxPool3dTFPadding(
            kernel_size=(2, 2, 2), stride=(2, 1, 1), padding='SAME')
        # Mixed 5
    ...
    def forward(self, inp):
        ...
        source_1 =  out = self.mixed_3c(out)         # this layer for 38x38 scale of fitting bounding boxes
        out = self.maxPool3d_4a_3x3(out)
        out = self.mixed_4b(out)
        out1 = self.mixed_4c(out)
        out = self.mixed_4d(out1)
        out = self.mixed_4e(out)
        out = self.mixed_4f(out)
        out = self.maxPool3d_5a_2x2(out)
        out = self.mixed_5b(out)
        source_2 = out = self.mixed_5c(out)          # this layer for 19x19 scale feature map
